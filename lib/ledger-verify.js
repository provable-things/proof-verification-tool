...
const verify = exports.verify = data => {
  ...
  let type;
  if (extractedCodeHash.equals(randomCodeHash)) type = 'random';else type = 'none';
...
const verifyRandom = exports.verifyRandom = data => {
  const proofBuffer = Buffer.from(data);
  const lenCert = proofBuffer.readUInt8(3 + 65 + 1) + 2;
...
const verifyLedger = exports.verifyLedger = data => {
      ...
      case 'random':
        {
          ...
          if (result) status = ['success', 'random valid'];else status = ['success', 'random invalid'];
