...
const getProofType = exports.getProofType = proof => {
  ...
  const compareProof = _ramda2.default.curry((proof, proofStructure) => {
    ...
    if (proofHeader === proofStructure.content) return proofStructure.proofName;
...
const findExtensionProof = message => {
  let extensionType = '';
  if (message !== null && message !== undefined && (0, _computationVerify.isComputationProof)(message)) extensionType = 'computation';else extensionType = 'proofType_NONE';
...
const verifyProof = exports.verifyProof = (() => {
  var _ref = _asyncToGenerator(function* (proof, callback) {
    ...
    const parsedProof = {
      ...
      message: message,
      // $FlowFixMe
      proofId: _crypto2.default.createHash('sha256').update(proof).digest().toString('hex')
    };
