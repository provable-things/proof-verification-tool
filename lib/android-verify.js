...
let verifyAuthenticity = (() => {
  var _ref = _asyncToGenerator(function* (jws, googleApiKey) {
    const postData = { signedAttestation: jws };
    const res = yield request('https://www.googleapis.com/androidcheck/v1/attestations/verify?key=' + googleApiKey, { method: 'POST', json: postData });
    const text = yield res.text();

    const googleResponse = JSON.parse(text);
    if (!googleResponse.isValidSignature) throw new Error('verifyAuthenticity failed');
  });

  return function verifyAuthenticity(_x, _x2) {
    return _ref.apply(this, arguments);
  };
})();

let verifyAuthenticityV2 = (() => {
  var _ref2 = _asyncToGenerator(function* (jws, googleApiKey) {
    const postData = { signedAttestation: jws };
    const url = 'https://www.googleapis.com/androidcheck/v1/attestations/verify?key=' + googleApiKey;
    const res = yield request(url, { method: 'POST', body: JSON.stringify(postData) });
    const text = yield res.text();

    if (res.status != 200) throw new Error('Error status: ' + res.status + ' on verifyAuthenticity for key: ' + googleApiKey + ' ' + res.status);
  ...
  return function verifyAuthenticityV2(_x3, _x4) {
...
const getCertificateChain = exports.getCertificateChain = encodedChain => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < cert.length; j += 64) out += cert.slice(j, j + 64) + '\n';
...
function verifyPayload(jwsPayload, response, requestID, signature, apkDigest, apkCertDigest, version) {
  ...
  if (version === 'v1') md.updateString(requestID);else if (version === 'v2') md.updateHex(requestID.toString('hex'));else throw new Error('version unsupported');

  const digest = md.digest();
  const nonce = Buffer.from(digest, 'hex').toString('base64');

  if (jwsPayloadJSON.nonce !== nonce.toString('base64')) throw new Error('verifyPayload failed: unexpected nonce');

  if (jwsPayloadJSON.apkPackageName !== 'it.oraclize.androidproof') throw new Error('verifyPayload failed: unexpected package name');

  if (jwsPayloadJSON.apkDigestSha256 !== apkDigest) throw new Error('verifyPayload failed: wrong apk hash');

  if (jwsPayloadJSON.apkCertificateDigestSha256[0] !== apkCertDigest) throw new Error('verifyPayload failed: wrong signing certificate hash');

  if (jwsPayloadJSON.basicIntegrity !== true) throw new Error('verifyPayload failed: SafetyNet basicIntegrity is false');
}
...
function verifyAttestationCertChain(leafCert, intermediateCert, rootCert, pemInter, pemRoot) {
  ...
  if (!intSig.verify(leafCertificateSignature) || !rootSig.verify(intCertificateSignature)) throw new Error('verifyAttestationCertChain failed');
}
...
function verifyAttestationParams(leafCert, attestationParams) {
  ...
  const RootOfTrust = asn.define('RootOfTrust', function () {
    this.seq().obj(this.key('verifiedBootKey').octstr(), this.key('deviceLocked').bool(),
    // $FlowFixMe
  ...
  const AuthorizationList = asn.define('AuthorizationList', function () {
    this.seq().obj(this.key('purpose').optional().explicit(1).setof(Int), this.key('algorithm').optional().explicit(2).int(), this.key('keySize').optional().explicit(3).int(), this.key('digest').optional().explicit(5).setof(Int), this.key('padding').optional().explicit(6).setof(Int), this.key('ecCurve').optional().explicit(10).int(), this.key('rsaPublicExponent').optional().explicit(200).int(), this.key('activeDateTime').optional().explicit(400).int(), this.key('originationExpireDateTime').optional().explicit(401).int(), this.key('usageExpireDateTime').optional().explicit(402).int(), this.key('noAuthRequired').optional().explicit(503).null_(), this.key('userAuthType').optional().explicit(504).int(), this.key('authTimeout').optional().explicit(505).int(), this.key('allowWhileOnBody').optional().explicit(506).null_(), this.key('allApplications').optional().explicit(600).null_(), this.key('applicationId').optional().explicit(601).octstr(), this.key('creationDateTime').optional().explicit(701).int(), this.key('origin').optional().explicit(702).int(), this.key('rollbackResistant').optional().explicit(703).null_(), this.key('rootOfTrust').optional().explicit(704).use(RootOfTrust), this.key('osVersion').optional().explicit(705).int(), this.key('osPatchLevel').optional().explicit(706).int(), this.key('attestationChallenge').optional().explicit(708).int(), this.key('attestationApplicationId').optional().explicit(709).octstr());
  });

  const KeyDescription = asn.define('KeyDescription', function () {
    this.seq().obj(this.key('attestationVersion').int(),
    // $FlowFixMe
    this.key('attestationSecurityLevel').enum({ 0: 'Software', 1: 'TrustedEnvironment' }), this.key('keymasterVersion').int(),
    // $FlowFixMe
    this.key('keymasterSecurityLevel').enum({ 0: 'Software', 1: 'TrustedEnvironment' }), this.key('attestationChallenge').octstr(), this.key('reserved').octstr(), this.key('softwareEnforced').use(AuthorizationList), this.key('teeEnforced').use(AuthorizationList));
  });
  const buffer = Buffer.from(value, 'hex');
  const keyInfo = KeyDescription.decode(buffer, 'der');

  if (String(keyInfo.keymasterVersion) !== attestationParams.keymasterVersion) throw new Error('verifyAttestationParams failed: keymasterVersion mismatch');

  if (String(keyInfo.attestationSecurityLevel) !== attestationParams.attestationSecurityLevel) throw new Error('verifyAttestationParams failed: attestationSecurityLevel');

  if (String(keyInfo.keymasterSecurityLevel) !== attestationParams.keymasterSecurityLevel) throw new Error('verifyAttestationParams failed: keymasterSecurityLevel mismatch');

  if (String(keyInfo.attestationChallenge) !== attestationParams.attestationChallenge) throw new Error('verifyAttestationParams failed: attestationChallenge value mismatch');

  if (String(keyInfo.teeEnforced.purpose) !== attestationParams.teeEnforced.purpose) throw new Error('verifyAttestationParams failed: key purpose mismatch');

  if (String(keyInfo.teeEnforced.algorithm) !== attestationParams.teeEnforced.algorithm) throw new Error('verifyAttestationParams failed: key algorithm type mismatch');

  if (String(keyInfo.teeEnforced.digest) !== attestationParams.teeEnforced.digest) throw new Error('verifyAttestationParams failed: key digest mismatch');

  if (typeof keyInfo.teeEnforced.ecCurve !== 'undefined' && String(keyInfo.teeEnforced.ecCurve) !== attestationParams.teeEnforced.ecCurve) throw new Error('verifyAttestationParams failed: ecCurve mismatch');

  if (String(keyInfo.teeEnforced.origin) !== attestationParams.teeEnforced.origin) throw new Error('verifyAttestationParams failed: key was not generated on device');
}
...
const verify = exports.verify = (() => {
  var _ref3 = _asyncToGenerator(function* (data, version) {
    ...
    if (version === 'v1') {
      ...
      encodedChain = jsonSettings.v1.androidCertChain.map(function (cert) {
        return Buffer.from(cert, 'base64');
      });
      pemEncodedChain = encodedChain.map(function (chain) {
        return getCertificateChain(chain);
      });
    } else if (version === 'v2') {
      settings = jsonSettings.v2;
      requestID = androidProof.requestID.toString('hex');
      encodedChain = jsonSettings.v2.androidCertChain.map(function (cert) {
        return Buffer.from(cert, 'base64');
      });
      pemEncodedChain = encodedChain.map(function (chain) {
        return getCertificateChain(chain);
    ...
    for (let k = 0; k < encodedChain.length; k++) {
      ...
      if (pemEncodedChain === null) {
        ...
        try {
          ...
          if (e.message === 'verifyAttestationParams failed: attestationSecurityLevel') status = e.message;else throw e;
        }
      ...
      if (!respVer) {
        if (k == encodedChain.length - 1) throw new Error('verifyResponseSignature failed');
        continue;
      } else {
        for (let j = 0; j < apkDigest.length; j++) {
          for (let i = 0; i < apkCertDigest.length; i++) {
            try {
              ...
              if (err.message !== 'verifyPayload failed: wrong signing certificate hash' && err.message !== 'verifyPayload failed: wrong apk hash') throw new Error('verifyPayload failed: apk hash or signing cert hash mismatch');
            }
          }
        }
        if (!verifySignature(jws, googleCert)) throw new Error('verifySignature failed');
        for (let i = 0; i < googleApiKey.length; i++) {
          try {
            let result = null;
            if (version === 'v1') result = yield verifyAuthenticity(jws, googleApiKey[i]);else if (version === 'v2') result = yield verifyAuthenticityV2(jws, googleApiKey[i]);else throw new Error('version unsupported');
            if (result) {
              status = '';
              break;
            }
          } catch (err) {
            if (i == googleApiKey.length - 1) throw new Error('verifyAuthenticity failed');
          }
        }
  ...
  return function verify(_x5, _x6) {
    return _ref3.apply(this, arguments);
  };
})();

const verifyAndroid = exports.verifyAndroid = (() => {
  var _ref4 = _asyncToGenerator(function* (data, version) {
    let status = null;
    try {
      const cborEncodedData = data.slice(3);
      const buf = Buffer.from(cborEncodedData.buffer);
  ...
  return function verifyAndroid(_x7, _x8) {
