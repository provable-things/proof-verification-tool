...
exports.servers.push([oraclize5, oraclize6,oraclize7b]);
...
exports.validateServer = function (server, type, mainPubKey) {
	try {
		...
		json = getJSON(res);
		var pubKey = checkGetConsoleOutput(json.GetConsoleOutputResponse, notaryServer.instanceId, args.launchTime, type, mainPubKey.pubkey);
	...
	try {
		...
		switch (type) {
			case 'main':
				...
				break;
			case 'sig':
				beginMark = consoleOutputStr.search('TLSNotary signing server pubkey:');
				assert(beginMark !== -1);
				pubKey = getPubKeyFromOutput(consoleOutputStr, beginMark);
				assert(pubKey.length > 0);
				...
				assert(mainPubKey === importedPubKey);
				break;
			default:
...
function checkDescribeInstanceAttribute(json, instanceId) {
	try {
		...
		console.log('checkDescribeInstanceAttribute failed', e.stack);
