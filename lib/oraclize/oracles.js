...
const snapshotIdV1Main = ['snap-cdd399f8']; // changed
const snapshotIdV1Sig = ['snap-00083b35']; // changed
const imageIDV1Main = ['ami-5e39040c']; // changed
const imageIDV1Sig = ['ami-88724fda']; // changed
...
const snapshotIdV2 = ['snap-03bae56722ceec3f0', 'snap-0fa4d717595514c9e', 'snap-0aae1b6ca8c4b0f8d', 'snap-001724fd6e3219a8b', 'snap-0d6e71fc4113c1d7f', 'snap-0d65665f4afaf28f8', 'snap-0f2a4af7bbcaf3374', 'snap-03a7cfbcc835aae69'];

const imageIdV2 = ['ami-1f447c65', 'ami-0900465cc4d168b18', 'ami-05fd672d486340db9', 'ami-004bb38dca2efe5cf', 'ami-0fc7d2d0c4a3fe3f5', 'ami-01887359c36e2e246', 'ami-0d9bdb20c502dbb32', 'ami-01fe7595953c2753b'];
...
const validateServer = (() => {
  var _ref2 = _asyncToGenerator(function* (server, type) {
    try {
      ...
      var res = yield request(notaryServer.DI);
      if (res.status !== 200) {
        throw new Error('aws_await request_failed');
      }
      var json = yield getJSON(res);
...
function checkDescribeInstances(jsonInput, instanceId, IP, type) {
  try {
    ...
    assert(imageId.includes(currentInstance.imageId.toString())); // changed: assert(currentInstance.imageId.toString() === imageId);
...
function checkDescribeVolumes(json, instanceId, volumeId, volAttachTime, type) {
  try {
    ...
    assert(snapshotId.includes(volume.snapshotId.toString())); // changed: assert(volume.snapshotId.toString() === snapshotId);
