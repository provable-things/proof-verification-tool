...
const parseProof = (() => {
  var _ref = _asyncToGenerator(function* (path) {
    ...
    try {
      ...
      if (!verifiedProof.mainProof.isVerified) throw new Error();
    } catch (error) {
    ...
    if (_ramda2.default.contains(flags.saveMessage, _process2.default.argv)) {
      if (typeof verifiedProof.message === 'string') yield (0, _helpers.writeFileAsync)(saveOutputPath(), verifiedProof.message);else yield (0, _helpers.writeFileAsync)(saveOutputPath(), Buffer.from((0, _index.getMessageContent)(verifiedProof.message, true)), 'binary');
    }
  });
