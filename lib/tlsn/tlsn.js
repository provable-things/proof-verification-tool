...
//#constants
const md5_hash_len = 16;
const sha1_hash_len = 20;
const aes_block_size = 16;
const tls_ver_1_0 = [3, 1];
const tls_ver_1_1 = [3, 2];
const tls_versions = [tls_ver_1_0, tls_ver_1_1];
//#record types
...
var tlsn_cipher_suites = [{ 47: ['AES128', 20, 20, 16, 16, 16, 16] }, { 53: ['AES256', 20, 20, 32, 32, 16, 16] }, { 5: ['RC4SHA', 20, 20, 16, 16, 0, 0] }, { 4: ['RC4MD5', 16, 16, 16, 16, 0, 0] }];
...
function prepare_pms(modulus, tryno) {
  ...
  return pms_session.sckt.connect().then(function () {
...
function negotiate_crippled_secrets(tlsn_session) {
  ...
  var s = tlsn_session;
  var cs_cr_sr_hmacms_verifymd5sha = [].concat(s.chosen_cipher_suite, s.client_random, s.server_random, s.p_auditee.slice(0, 24), s.handshake_hash_md5, s.handshake_hash_sha);
  return send_and_recv('cs_cr_sr_hmacms_verifymd5sha', cs_cr_sr_hmacms_verifymd5sha, 'hmacms_hmacek_hmacverify').then(function (reply_data) {
...
function get_certificate(server, port) {
  ...
  return probe_session.sckt.connect().then(function () {
...
function start_audit(modulus, certhash, name, port, headers, ee_secret, ee_pad_secret, rsapms2) {
  ...
  return tlsn_session.sckt.connect().then(function () {
    ...
    fullresp = [].concat(tlsn_session.unexpected_server_app_data_count, tlsn_session.unexpected_server_app_data_raw, response);
    ...
    return [tlsn_session.chosen_cipher_suite, tlsn_session.client_random, tlsn_session.server_random, tlsn_session.pms1, tlsn_session.pms2, tlsn_session.server_certificate.chain, tlsn_session.tlsver, tlsn_session.initial_tlsver, fullresp.length, fullresp, tlsn_session.IV_after_finished.length, tlsn_session.IV_after_finished, chosen_notary.modulus.length, signature, commit_hash, chosen_notary.modulus, plaintext, time];
  });
}
...
function tls_record_fragment_decoder(t, d, args) {
  ...
  if (conn) {
    if (ignore_mac) {
      //means we won't check it now, but store to be checked later
...
TLSHandshake.prototype.__init__ = function (serialized, handshake_type) {
  ...
  if (serialized) {
    ...
    assert([h_ch, h_sh, h_shd, h_cert, h_cke, h_fin].indexOf(this.handshake_type) > -1, 'Unrecognized or unimplemented handshake type');
...
TLSClientHello.prototype.__init__ = function (args) {
  ...
  if (serialized) {
    print('Not implemented instantiation of client hello', 'with serialization; this is a client-only', ' TLS implementation');
  } else {
    if (client_random) {
...
TLSServerHello.prototype.__init__ = function (args) {
  ...
  if (serialized) {
    ...
    assert(cs_keys.indexOf(this.cipher_suite) > -1, 'Server chosen cipher suite not in TLS Notary allowed list, it was: ' + this.cipher_suite.toString());
...
TLSNClientSession.prototype.process_server_hello = function (handshake_objects) {
  ...
  assert(handshake_types.indexOf(h_sh) >= 0 && handshake_types.indexOf(h_cert) >= 0 && handshake_types.indexOf(h_shd) >= 0, 'Server failed to send server hello, certificate, server hello done');
  ...
  if (this.server_hello.tlsver.toString() !== this.tlsver.toString()) {
    if (this.server_hello.tlsver.toString() === [0x03, 0x01].toString() && this.tlsver.toString() === [0x03, 0x02].toString()) {
...
TLSNClientSession.prototype.set_encrypted_pms = function () {
  assert(this.enc_first_half_pms && this.enc_second_half_pms && this.server_modulus, 'failed to set enc_pms, first half was: ' + this.enc_first_half_pms.toString() + ' second half was: ' + this.enc_second_half_pms.toString() + ' modulus was: ' + this.server_modulus.toString());
...
TLSNClientSession.prototype.set_enc_second_half_pms = function () {
  ...
  ///XXX JS mod exp
  this.enc_second_half_pms = pow(tlsn_utils.ba2int('\x01' + '\x01' * ones_length + this.auditor_padding_secret + '\x00' * 25 + this.pms2), this.server_exponent, this.server_modulus);
};
...
TLSNClientSession.prototype.get_p_value_ms = function (ctrprty) {
  ...
  assert(this.server_random && this.client_random && this.chosen_cipher_suite, 'server random, client random or cipher suite not set.');
...
TLSNClientSession.prototype.check_server_ccs_finished = function (provided_p_value) {
  ...
  assert(this.server_finished.verify_data.toString() == verify_data_check.toString(), 'Server Finished record verify data is not valid.');
  return true;
};
...
TLSNClientSession.prototype.process_server_app_data_records = function () {
  ...
  assert(this.server_response_app_data.length > 0, 'Could not process the server response, no ciphertext found.');
  var plaintexts = [];
  ...
  for (var i = 0; i < this.server_response_app_data.length; ++i) {
    ...
    if (ciphertext.typename === 'TLSAppData') {
      ...
      throw 'Server response contained unexpected record type: ', ciphertext.typename;
    }
