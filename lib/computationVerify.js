...
module.exports = {
	verifyComputation: function (raw_html, archive) {
		try {
			...
			var oraclize_result = awsOutput.match(/^ORACLIZE_RESULT:[\s\S]*ORACLIZE_/m)[0];
			oraclize_result = oraclize_result.substr(16, oraclize_result.length-16-9).split("\r\r\n").join("");
			var oraclize_doc = awsOutput.match(/^ORACLIZE_DOC:[\s\S]*ORACLIZE_/m)[0];
			oraclize_doc = oraclize_doc.substr(13, oraclize_doc.length-13-9).split("\r\r\n").join("");
			var oraclize_sig = awsOutput.match(/^ORACLIZE_SIG:[\s\S]*$/m)[0];
			oraclize_sig = oraclize_sig.substr(13, oraclize_sig.indexOf("[")-13).split("\r\r\n").join("");
			var oraclize_userdata = awsOutput.match(/^ORACLIZE_USERDATA:[\s\S]*ORACLIZE_/m)[0];
			oraclize_userdata = oraclize_userdata.substr(18, oraclize_userdata.length-18-9).split("\r\r\n").join("");
